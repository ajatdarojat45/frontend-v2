...
export async function processModelFromUrl(modelId: number, modelUrl: string): Promise<RhinoFileData> {
  try {
    ...
    throw new Error(`Failed to process model: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
...
export async function processModelFromData(
  ...
  try {
    ...
    throw new Error(`Failed to process model data: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
