...
// @ts-ignore - Three.js loader types may not be available
...
export async function parseFileAsRhinoDoc(fileData: ArrayBuffer): Promise<any> {
  try {
    ...
    throw new Error(`Failed to parse rhino document: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
...
export async function parseFileAsThreeObject(fileData: ArrayBuffer): Promise<THREE.Object3D> {
  return new Promise((resolve, reject) => {
    try {
      ...
      reject(new Error(`Failed to initialize 3DM loader: ${error instanceof Error ? error.message : 'Unknown error'}`))
    }
  })
}

export function extractLayerInfo(rhinoDoc: any): Array<RhinoLayerData> {
...
export function extractObjectAttributes(rhinoDoc: any): Array<RhinoObjectAttribute> {
